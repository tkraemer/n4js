...
class RuntimeEnvironmentResolutionPluginUITest extends RuntimeEnvironmentResolutionTest {
	...
	@Override
	override protected createProjectWithManifest(String artifactId, String manifestContent) {
		val project = workspace.root.getProject(artifactId)
		...
		try {
			...
			LOGGER.error('''Error while creating manifest file for project: '«artifactId»'.''')
			throw propagate(e)
		} finally {
			if (null !== is) {
				try {
		...
		if (null !== exc) {
			throw new RuntimeException('Error while closing the stream for the N4 manifest file.', exc)
		}
