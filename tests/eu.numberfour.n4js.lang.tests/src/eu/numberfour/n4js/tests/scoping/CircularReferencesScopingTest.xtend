...
class CircularReferencesScopingTest implements N4Scheme {
	...
	def void setUp() {
		...
		brotherURI = rs.URIConverter.normalize(URI.createURI("src/eu/numberfour/n4js/tests/scoping/Brother.n4js"))
		sisterURI = rs.URIConverter.normalize(URI.createURI("src/eu/numberfour/n4js/tests/scoping/Sister.n4js"))
		childURI = rs.URIConverter.normalize(URI.createURI("src/eu/numberfour/n4js/tests/scoping/Child.n4js"))
	...
	def void testCircularDependencies() {
		...
		assertEquals(1, resourceDescriptions.getExportedObjects(TypesPackage.Literals.TMODULE,
			QualifiedName.create("eu", "numberfour", "n4js", "tests", "scoping", "Sister"), false).size)
