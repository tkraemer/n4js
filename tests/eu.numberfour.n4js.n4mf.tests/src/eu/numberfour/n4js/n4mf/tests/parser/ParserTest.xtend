 ...
 */
package eu.numberfour.n4js.n4mf.tests.parser
...
//	& 	('ModuleFilters'	'{' moduleFilters+=ModuleFilter+ '}')?
	...
	@Test def void testCompleteExample() {
		val project = '''
			...
			ProjectDependencies {
				eu.numberfour:my.project.one (0.0.1,0.0.2] compile,
				eu.numberfour:my.project.one 1.0 compile
			}
		'''.parse
		val errors = project.eResource.errors
		assertTrue(errors.toString, errors.empty)
	...
	@Test def void testProjectDependencyWithoutScope() {
		val project = '''
			...
			ProjectDependencies {
				eu.numberfour:my.project.one (0.0.1,0.0.2]
			}
		'''.parse
		val errors = project.eResource.errors
		assertTrue(errors.toString, errors.empty)
	...
	@Test def void testProjectDependencyWithoutVendor() {
		...
		assertEquals("eu.numberfour", project.allProjectDependencies.last.project.vendorId)
	}
