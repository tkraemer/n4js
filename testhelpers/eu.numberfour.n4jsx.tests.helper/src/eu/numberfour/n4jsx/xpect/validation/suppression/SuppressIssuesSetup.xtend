 ...
 */
package eu.numberfour.n4jsx.xpect.validation.suppression
...
import eu.numberfour.n4js.xpect.validation.suppression.SuppressIssuesSetupRoot
import eu.numberfour.n4jsx.validation.IssueCodes
...
class SuppressIssuesSetup extends IssuesByLineProvider {
	...
	private final Collection<String> suppressedIssueCodes = Collections.unmodifiableCollection(
		Arrays.asList(IssueCodes.REACT_ELEMENT_NOT_FUNCTION_OR_CLASS_ERROR));
	 ...
	 * 
	 */
	new(@ThisResource XtextResource resource, ISetupInitializer<SuppressIssuesSetupRoot> setupInitializer) {
		super(resource);
	...
	override collectIssuesByLine() {
		...
		if (issuesByLine === null) {
			val validator = getResource().getResourceServiceProvider().
				getResourceValidator() as TestingResourceValidator;
		...
		if (!suppressedIssueCodes.empty) {
			...
			issuesByLine.entries.filter[entry|!includeIssue(entry.value)].forEach [entry |
				filteredResults.remove(entry.key, entry.value)
			];
