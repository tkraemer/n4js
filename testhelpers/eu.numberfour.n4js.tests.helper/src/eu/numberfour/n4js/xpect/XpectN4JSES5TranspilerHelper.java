...
public class XpectN4JSES5TranspilerHelper {
	...
	private String jsModulePathToRun(Script script) {
		ArrayList<String> pathSegements = newArrayList(getProjectArtifactId(script));
	...
	private File createTempJsFileWithScript(final Script script, final boolean replaceQuotes) throws IOException {
		...
		final String artifactId = getProjectArtifactId(script);
		Path parentPath = createDirectory(getTempFolder(), artifactId); // TODO IDE-
	...
	private String getProjectArtifactId(final Script script) {
		return script.getModule().getProjectArtifactId();
	}
}
