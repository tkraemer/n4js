...
Grammar and semantics of import statement is described in <<_import-statement,Import Statement>>; of export
statement described in <<_export-statement,Export Statement>>.
...
[horizontal]
`annotations` :: Arbitrary annotations, see <<_annotations,Annotations>> and below for details.
...
Pseudo properties to be set via annotations are explained in <<_annotations,Annotations>>.
...
They basically define keys <<_primitive-pathselector-and-i18nkey,Primitive Pathselector I18nKey>> with their values.
...
API projects may use N4JS DI (<<_dependency-injection,Dependency Injection>>) language features which require Implementation projects to provide DI-compatible behaviour in order to allow a Client (implemented against an API project) to be executed with a given Implementation project.
