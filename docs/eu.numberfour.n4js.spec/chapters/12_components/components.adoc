...
project name and not the complete module specifier (see <<import-statement-semantics>>).
...
<<_implementation-of-external-declarations>>
...
`expandedPath` :: Pseudo property consists of the project name and project version of the module followed by the path, the concrete syntax is:
+
...
. Collect all implementation projects for `A` (i.e. projects that specify `A` in their manifest under `ImplementedProjects`).
