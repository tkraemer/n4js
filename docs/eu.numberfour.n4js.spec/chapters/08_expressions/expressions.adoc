...
`pass:[++]` and `pass:[+=]` are deduced to `pass:[+]` (and simple assignment).
...
expression with `@This(B)`, access to the receiver of type B is enabled.
...
++++
\beginalign
\infer{\type{PropertyAccessExpression}\ expr: T}{\typeEnvAdd \typeSubs(R) \entails expr.target : R & \typeEnv \entails expr.property : T}
\endalign
++++
...
The type inference and constraints for postfix operators `pass:[++]` and `--`, cf. cite:[ECMA11a(S11.3.1,p.70)], cite:[ECMA11a(S11.3.1,p.70)], are defined similarly to their prefix variants (unary expressions), see <<Unary Expression>>.
...
* If $u.expression = PropertyAccess \; pa(p) \land pa.isDotAccess \to $ both $get$ _p_ and $set$ _p_ must be defined. task:IDE-737[]
--
