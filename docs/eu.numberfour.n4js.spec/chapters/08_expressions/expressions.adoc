...
** a string literal, e.g. `['myProp']`,
** a built-in symbol, for example as in `[Symbol.iterator]`, or
** a literal of a `@StringBased` enum.
...
// TODO check \tag{\S 11.8.1-6} in math blocks
...
[math]
++++
\infer{\tee left\ \lstnfbnf{'@='} right: T}{\tee left\ \lstnfbnf{'@'} right: T} \tag{\S 11.13.1}
++++
...
[math]
++++
\infer{\tee cexpr: T}{n=|cexpr.exprs|, \tee cexpr.exprs_n:T} \tag{\S11.14}
++++
