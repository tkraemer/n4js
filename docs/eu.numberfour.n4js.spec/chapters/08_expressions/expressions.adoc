...
The first requirement of the above definition rules out property access expressions that do not directly points to their target classifier or enum, as shown in the following example:
...
A direct property access is the only form of property access allowed in compile-time expressions, cf. <<compile-time-expressions>>.
...
1.  A limited form of computed-name index-access is allowed in N4JS mode.
In case the receiver is of dynamic type, the index can be any expression task:GH-238[].
Otherwise, the index-access is limited in that the index must be a compile-time expression (cf. <<compile-time-expressions>>).
Feasible targets of such accesses are the same as for dot-access.
2.  Additionally, an index-access expression is allowed when targeting (a) one of the types Array, ArgumentType, string, String, Iterable (including their subtypes), (b) `Object` (**not** including subtypes of `Object` and not for `pass:[~Object]` and `pass:[~~Object]` ), or (c) a dynamic type.
+
...
must be of a certain value. In any case, if an expression has operands, it is a compile-time expression only if
all operands are compile-time expressions.
...
*** the literal of a `@StringBased` enum, or
*** a const field, iff its initializer expression is a compile-time expression.
