...
--
If an identifier $i$ is accessed, the bound declared element
$D$ must be readable if it is not used on the left-hand side
...
&\infer{\type{RegularExpressionLiteral}: \type{RegExpr}}{} \tag{\S7.8.5}$
...
(cf. 4.b/c): +
$\forall & pa \in ol.propertyAssignments, pa$\nexists & pgsd \in ol.propertyAssig$& \mu(pgsd)\neq\type{PropertyNameValuePair} \land pgsd.name = pa.name$
+
...
expression. The type if a parenthesized expression $pe$ is
inferred as follows:
...
.  If $C$ is not a constructor type, it must be a
...
to 1) The type of an abstract class `A` is $\type{type\{A\}}$.
...
&\infer{\tee \lstnfbnf{'!'}\ expression: \type{boolean}}{}  \tag{\S 11.4.9}$
...
---
\infer
{\typeEnv \entails \type{MultiplicativeExpression} \expectType \type{Expression}: \type{number}}{}
--
...
---
\infer
{\typeEnv \entails \type{MultiplicativeExpression}: \type{number}}{}
---
...
---
\infer
{\typeEnv \entails \type{AdditiveExpression}\ e \expectType \type{Expression}: \type{number}}
...
\infer[un]{un(expr)}{un\tee expr.lhs \lor un\tee expr.rhs}
---
...
---
\infer
{\tee e: \type{string}}{e.op='+' & \lnot(nb(e.lhs)\land nb(e.rhs)) & \lnot(un(e) \land (nb(e.lhs)\lor nb(e.rhs))}\\
&\infer{\tee e: \type{number}}{}\\
---
...
[math]
---
&\infer{\tee\ (\lstnfbnf{Expression ('<<'|'>>'|'>>>')\ Expression}): \type{number}}{}   \tag{\S 11.7.1/2} \\
---
...
task:IDE-345[]
For a given relational expression $e$ with
$e.op = \lstnfjs{in}$, the following constraints must hold:
...
$&\infer{\tee lhs\ \lstnfbnf{('<'|'<='|'>'|'>='|'instanceof'|'in')}\ rhs\ : \type{boolean}}{}    \tag{\S 11.8.1-6}$
...
Neither $\lstnfjs{? extends A}\subtype \lstnfjs{B}$ nor
$\lstnfjs{B} \subtype \lstnfjs{? extends A}$ is true. This
...
$&\infer{\tee lhs\ \lstnfbnf{('=='|'!='|'==='|'!==')}\ rhs\ : \type{boolean}}{}  \tag{\S 11.9}$
...
---
\infer
{\tee \type{BitwiseBitwiseExpression}\ \expectType\ \type{Expression}: \type{number}}{}
---
...
$&\infer{\tee\ (\lstnfbnf{Expression ('&'|'^'|'|')\ Expression}): \type{number}}{}   \tag{\S 11.10}$
...
[math]
---
\infer{\tee\ lhs\ \lstnfbnf{'&&'|'||'}\ rhs: \type{\union*}\tee lhs, \tee rhs \type{\}}}{}
---
...
$assignExpr$, the left-hand side must be writeable or a
final data field and the assignment must be in the constructor. Let
$v$ be the bound variable (or field) with
$bind(assignExpr.left, v)$
$
v.writeable \lor
    v.final \land
...
$&\infer{\tee left\ \lstnfbnf{'='} right: T}{\tee right: T}  \tag{\S 11.13.1}$
...
---
\infer
{\tee left\ \lstnfbnf{'@='} right: T}{\tee left\ \lstnfbnf{'@'} right: T} \tag{\S 11.13.1}
---
...
---
\infer
{\tee cexpr: T}{n=|cexpr.exprs|, \tee cexpr.exprs_n:T} \tag{\S11.14}
---
...
Apart from the use of keyword `super` in wildcards of type expressions (cf. <<_type_expressions>>), there are two use cases for keyword `super`: super member access and super
constructor calls.
...
--
If the super literal $s$ is used to access the super constructor of a
...
+
3.  The containing statement $stmtExpr$ must be directly
contained in a constructor body:
...
containing statement $exprStmt$. +
Let $si$ be the index of $exprStmt$ in the
constructor body: +
$exprStmt.container.stmts_{si}=exprStmt$. +
Then, the following constraint must
holdfootnote:[$e \in^* c$ is the transitive version of
$e \in c$, that is, it $e$ directly or
indirectly contained in $c$.]
