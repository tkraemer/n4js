...
++++
\beginalign
\infer{\tee \lstnfbnf{"this"}: \tsNom T}{f=\lstnfbnf{"this"}.containingFunctionOrAccessor \\
f.hasAnnotation(\lstnfbnf{"@This"}) \spc T = f.annotation\lstnfjs{["@This"]}}\\
\endalign
++++
...
+
[%hardbreaks]
$mode=\lenum{strict} \to \spc \forall pa \in ol.propertyAssignments, pa.isData:$
...
+
[%hardbreaks]
$mode=\lenum{n4js} \to \spc \forall pa \in ol.propertyAssignments:$
...
[math]
++++
\spc pae.isDotAccess \land \lnot R.dyn \to \\
...
++++
\spc pae.isDotAccess \land \lnot R.dyn \to\\
\spc (\exists m \in pae.target.type.properties : m.name=pae.name) \to  \alpha(pae, m)
++++
+
...
$\spc \to \tee pae.index \subtype[number]$
...
++++
\beginalign
\infer{\tee p: T}{\lnot p.target.dyn \lor p.index.type \subtype[number] \spc \tee p.target: \type{Array<T>}} \\
...
Or in other words: Only instantiable classes have an inferred type of $constructor\{..\}$.
...
* If $u.expression = PropertyAccess \; pa(p)  \land  pa.isDotAccess$ $\to$ both $get$ _p_ and $set$ _p_ must be defined. task:IDE-768[]
...
[math]
++++
\spc\infer{\tee\ (\lstnfbnf{Expression ('<<'|'>>'|'>>>')\ Expression}): \type{number}}{}  \\
++++
...
+
[%hardbreaks]
$\spc\infer{\tee  lhs\ \lstnfbnf{('<'|'<='|'>'|'>=')}\  rhs\  \expectType\ lhs: T}{ \tee rhs: T \spc T\in \{\type{number,string,boolean}\}}$
$\spc\infer{\tee  lhs\ \lstnfbnf{('<'|'<='|'>'|'>=')}\  rhs\  \expectType\ lhs: T}{ \tee rhs: O \spc O \not\in \{\type{number,string,boolean}\} \spc T=\type{\union{number,string,boolean}}  } $
$\spc\infer{\tee  lhs\ \lstnfbnf{('<'|'<='|'>'|'>=')}\  rhs\  \expectType\ rhs: T}{ \tee lhs: T \spc T\in \{\type{number,string,boolean}\}} $
$\spc\infer{\tee  lhs\ \lstnfbnf{('<'|'<='|'>'|'>=')}\  rhs\  \expectType\ rhs: T}{ \tee lhs: O \spc O \not\in \{\type{number,string,boolean}\} \spc T=\type{\union{number,string,boolean}}  } $
...
is contained in the the first type rule, an object type reference footnote:[Includes interfaces, since an interface type reference is a subtype of object type reference: $\type{type\{Interface\}} <: \type{type\{Object\}}$]
...
\spc\infer{\tee  lhs\ \lstnfbnf{'instanceof'}\  rhs\  \expectType\ rhs: \type{type\{Object\}}}{} \\
\spc\infer{\tee  lhs\ \lstnfbnf{'instanceof'}\  rhs\  \expectType\ rhs: \type{type\{N4Enum\}}}{}
++++
...
[math]
++++
\spc\infer{\tee  lhs\ \lstnfbnf{'in'}\ rhs\  \expectType\ lhs: \type{\union{string,number}}}{}
++++
