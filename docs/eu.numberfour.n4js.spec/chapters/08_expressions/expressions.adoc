...
math:[$\to$] both math:[$get$] _p_ and
math:[$set$] _p_ must be defined. task:IDE-737[]
--
...
math:[$u.expression = PropertyAccess \; pa(p)  \land  pa.isDotAccess$]
math:[$\to$] both math:[$get$] _p_ and
math:[$set$] _p_ must be defined. task:IDE-768[]
...
--
For a given bitwise bitwise expression math:[$e$] the following constraints must hold
...
--
For a given conditional expression math:[$e$] with
$$e.expression.type: C,
...
indirectly contained in math:[$c$].]
