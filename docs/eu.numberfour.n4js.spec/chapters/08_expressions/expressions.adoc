...
$$\infer{\tee expr: T}{bind(expr.target, F) & F.returnType: T}$$
...
$$\infer{\typeEnv \entails \type{MultiplicativeExpression} \expectType \type{Expression}: \type{number}}{}$$
--
