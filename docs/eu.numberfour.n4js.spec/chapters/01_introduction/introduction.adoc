...
is the context containing (bound) type variables etc., $\entails$ can be read as `entails`.
...
// TODO missing type here

1.  Declared type: the type explicitly specified in the code, e.g., .
2.  Inferred type: the type inferred by the type inferencer, e.g., infers the type of s to `string`. I.e.
...
The explicitly declared type, this is usually a real property of the construct.
...
A pseudo property for elements with a $declaredType$ property.
