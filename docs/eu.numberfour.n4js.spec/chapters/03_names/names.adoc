...
$\alpha(r,D)$ instead of $\alpha_T(r,D)$.
...
Note that $alpha_m$ and $bind$ are different
...
Formally, we define $\alpha_m$ for a given reference
$r$ and member declaration $M$ as
follows:
footnote:[See <<_n4js_specific_classifiers,N4-Specific Classifiers>> for definitions of metatype properties. Note that $r.receiver$] always refers to a type declaration in the context of an expression as the receiver type of $r$. The declaring type of the member declaration is considered to be the receiver type of the member reference rather than the type that originally declares the member
declaration.]
...
If the type of the arguments is clear from the context, we simply write
$\alpha(r,M)$ instead of $\alpha_m(r,M)$.
