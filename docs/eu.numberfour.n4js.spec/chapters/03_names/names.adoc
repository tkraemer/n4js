...
<<fig:forbidden_shadowing,Fig Forbidden Shadowing>> shows nested lexical environments with named elements declared inside (all named ``x`` here), the forbidden cases are marked with arrows (the numbers at the left side refer to the numbers in <<IDE-8,Forbidden Shadowing>>.
