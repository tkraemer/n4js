...
possible to redefine (override etc.) these members (see <<_Redefinition-of-Members>>).
...
<<fig:forbidden_shadowing>> shows nested lexical environments with named elements declared inside (all named ``x`` here), the forbidden cases are marked with arrows (the
numbers at the left side refer to the numbers in <<IDE-8,Forbidden Shadowing>>.

[[fig:forbidden_shadowing]]
