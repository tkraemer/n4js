...
We define type erasure similar to Java cite:[Gosling12a(S4.6)] as 'mapping from types (possibly including parameterized types and type variables) to types (that are never parameterized types or type variables)'. We write $T$^o^ for the erasure of type _T_.

TIP: The notation $|T|$ used in cite:[Gosling12a] conflicts with the notation of cardinality of sets, which we use in case of union or intersection types for types as well. The notation used here is inspired by cite:[Crary02a], in which a mapping is defined between a typed language $\lambda$ to an untyped language $\lambda$^o^.
...
TIP: For type theory about union types, cite:[Pierce02a(15.7)] and cite:[Igarashi07a], other languages that explicitly support the notion of union type include Ceylon cite:[King13a(3.2.4/5)]
...
TIP: This is different from Ceylon ( cite:[King13a(3.2.3)]), in which the union is defined to be `the same type as` _A_.
Although the meaning of `same` is not clear, it is possibly used as a synonym for `equivalent`.
...
TIP: For type theory about intersection types, see cite:[Pierce02a(15.7)] and cite:[Laurent12a], other languages supporting explicit notion of intersection type include Ceylon cite:[King13a(3.2.4/5)].
...
[math]
++++
\exists C \in I.typeRefs: \mu(C)=`Class}) \to \nexists T \in I.typeRefs \setminus\{C\}: \mu(T)=`Class}
++++
//`
+
...
its contained types is a subtype of that type:

TIP: This rule is a generalization of the subtyping rules given in cite:[Laurent12a] Table 2, $\cap^1_l$ and $\cap^2_l$
...
types contained in the intersection type:

TIP: This rule is a generalization of the subtyping rules given in cite:[Laurent12a]Table 2, $\cap_r$
...
TIP: In Ceylon, for a given union type $U=T_1|T_2$ and intersection type $I=T_1\&T_2$ (with ’|’ is union and ’&’ is intersection),
...
. Static members of a type _T_ are actually members of the classifier type _type{T}_.
. The keyword `this` in a static method of a type _T_ actually binds to the classifier type _type{T}_.
. The constructor type ``constructor``_{T}_ is a subtype of the classifier type _type{T}_:
+
$\forall T:$ `constructor{T}` $<:$ `type{T}`
. If a class _B_ is a subtype (subclass) of a class _A_, then the classifier type _type{B}_ also is a subtype of _type{A}_:
+
[math]
++++
...
[math]
++++
\infer{\tee C: \lstnfjs{constructorC}}{\mu(C) \in { \type{classifierDefinition} } }
++++
