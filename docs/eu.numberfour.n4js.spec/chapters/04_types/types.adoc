...
<<fig-constructors-and-prototypes>> for two classes A and B in ECMAScript 2015 shows the constructors, prototypes, and the relations between them for the ECMAScript 2015 code shown in <<ex-constructors-and-prototypes>>.
...
NOTE: The diagram <<fig-constructors-and-prototypes>> shows plain ECMAScript 2015 only.
...
If we had defined `A` as `class A extends Object {}` in the listing above, then the constructor of `A` would have Object’s constructor as its prototype (depicted in as a dashed red arrow), which would make a more consistent overall picture.
