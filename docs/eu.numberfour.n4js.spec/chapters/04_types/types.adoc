...
[def]
--
A type $$J$$ is called a _join_ (or least common supertype, ) of a pair of types $$S$$
and _T_, written $$S \join T = J$$, if +
...
$$`(T extends B)} <: `A}$$ in which T is an
unbound type variable and A, B two classes with $$B <: A$$.
3.  In all other cases the result is false. +
This includes cases such as +
$$\[`B} <: `(T extends A)}\]$$
which is always false, even if $$B <: A$$ or +
$$`(T extends A)} <: `(S extends B)}$$ which
is always false, even if $$A = B$$.
...
* The erasure $$G°$$ of a parameterized type
...
* For two non-parameterized types $$T^0$$ and
$$S^0$$,
...
$$A\ \mbox{join}\  \lstnfjs{union\{}B,C\lstnfjs{\}} \neq A\ \mbox{join}\  B \ \mbox{join}\  C\$$
...
$$I'$$ containing the upper and lower bound of the elements of
$$I$$:
...
// TODO check this section for formatting / syntax
...
base type `N4StringBasedEnum`: $$\begin{gathered}\infer{ \tee \type{type\{E_S\}} \subtype \type{N4StringBasedEnum} }{}                \\
