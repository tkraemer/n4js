...
math:[$T$] actually is a union type of `Undef|$T$`. Default value:
math:[$false$].
...
[[cdVarianceChart]]
image::fig/cdVarianceChart.png[title="Cheat Sheet: Subtype Relation of Parameterized Types"]
...
The built-in object type `Function`, a subtype of `Object`, represents all functions, regardless of how they are defined 
(either via function expression, function declaration, or method declaration).
...
For the time being, however, an instance of `Function` can be invoked,
any number of arguments may be provided and the invocation may be parameterized with any number of type arguments (which will be ignored), 
i.e. <<IDE-101,Function Call Constraints>> and <<IDE-102,Parameterized Function Call Constraints>> do not apply.
