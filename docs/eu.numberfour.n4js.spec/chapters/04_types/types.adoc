...
[[ex:Type Examples, Class Hierarchy]]
// TODO example block
...
summarizes the type expression grammar. Depending on the context, not
all constructs are allowed. For example, the variadic modifier is only
allowed for function parameters.
...
[Type Variable as Upper Bound][ex:Type Variable as Upper Bound] Note
...
[F-bounded quantification][ex:F_bounded_quantification] Using a type
variable in the upper bound reference may lead to recursive definition.
...
[Type variables vs. existential
types][ex:TypeVariablesVsExistentialTypes]
...
Even though the upper bound of is a subtype of ’s upper bound (since
math:[$B <: A$]), we cannot infer that is a subtype of (line 15),
because there are valid concrete bindings for which this would not be
true: for example, if were bound to and to .
...
[Subtype Relation for Type
Variables][def:SubtypeRelationForTypeVariables] For two types
math:[$T, S$] of which at least one is a type variable, we define
...
References to generic types (cf. ) can be parameterized with type
...
the structural type, see for details.


*Pseudo Properties:*
//TODO: fix the following autoref
...
// TODO: Requirement [req:Parameterized_Type]
...
[Parameterized Type, Raw Type][def:Parameterized_Type] A parameterized
type reference math:[$R$] defines a parameterized type
...
In this case, variable refers to the _raw type_ . This is forbidden in
...
In Java, both assignments work with raw types, which is not really save.
To avoid problems due to different interpretations, usage of raw types
is not allowed in N4JS.footnote:[Although raw type usage is prohibited,
the N4JS validator interprets raw types according to the first case,
which may lead to consequential errors.]
...
[Type Conformance][def:Type_Conformance] We define type conformance for
non-primitive type references as follows:

// TODO: Fix adoc syntax highlighting:
[source]
----
...
[[ex:Subtyping with parameterized types]]
//TODO Example Block
...
shows the subtype relations of parameterized types (of a single generic
type), which can be used as a cheat sheet.

image::fig/cdVarianceChart.png[title="cdVarianceChart"]
Cheat sheet: subtype relation of parameterized types]

[[ex:Subtyping between different generic types]]
// TODO Example Block
Let classes
math:[$G$] and math:[$H$] be two generic classes where:
...
in G, e.g. and , the following code snippet will produce an error:

This is no surprise, as actually returns a type math:[$E_1$], which
...
[[req:12Join of Parameterized Types]]
// TODO Requirement
...
//TODO: Example
[[ex:Upper and lower bound of parameterized types]]
...
// Req 16 - Variadic and optional parameters
...
//TODO : requirements
//[req:Variadic_at_Runtime]
//17Variadic at Runtime
task:IDEBUG-106[]
...
// TODO: requirements # 19
// 19Union Type Subtyping Rules[req:Union_Type_Subtyping_Rules]
...
//TODO requirements:
//20Implicit simplification of union types[req:Simplification_Union_Type]
...
// TODO: Example block
//[Subtyping with union type][ex:Subtyping_with_union_type]
...
//Requirement
//21Union of union type[req:Union of union type]
...
//Requirement
//22Join with Union Type[req:Join_with_Union_Type]
...
//Requirement
//23Meet with Union Type[req:Meet_with_Union_Type]
...
//Requirement
//24Upper and Lower Bound of a Union Type[req:Upper_and_Lower_Bound_of_a_Union_Type]
...
//Requirement
//25Any type in union types[req:Any_type_in_union_types]
...
//Requirement
//26Redundant subtypes in union types[req:Redundant_subtypes_in_union_types]
...
27Intersection Type For a given intersection type math:[$I$], the
...
28Intersection Type Subtyping Rules[Intersection Type Subtyping Rules]
...
// TODO: requirements [Implicit simplification of intersection types][req:Simplification_Intersection_Type]
Let math:[$I$] be an
...
// TODO: Example Block
//[[ex:Subtyping with intersection type]]
...
//Requirement
// 29Join with Intersection Type[req:Join_with_intersection_Type]
...
30Meet with intersection Type[req:Meet_with_intersection_Type] The meet
...
//Requirement
//31Upper and Lower Bound of an Intersection Type[req:Upper_and_Lower_Bound_of_an_Intersection_Type]
...
//Requirement
// 32Any type in intersection types[req:Any_type_in_intersection_types]
...
//Requirement
//33Redundant supertypes in intersection types[req:Redundant_subtypes_in_intersection_types]
...
//Requirement
//34Composed Types as Bound of a Wildcard[req:Composed_Types_as_Bound_of_Wildcard]
...
//Requirement
//36Members of an Intersection Type
...
[[req:41String-Based Enum Type Rules]]
// TODO Requirement
...
// TODO: fix the following listings
