...
* `name`: Type variable, as type variable contains only an identifier, we use type
parameter instead of type variable (and vice versa) if the correct
element is clear from the context.
* `declaredUpperBound`: upper bound of the concrete type bound to this type variable, e.g. a super
class.
...
As a built-in type, the type `undefined` cannot be declared explicitly by the user by means of a type
expression. Note in ECMAScript there are three different use cases of `undefined`:
...
important to note that something that is not assigned a value is `undefined` but not `null`.
