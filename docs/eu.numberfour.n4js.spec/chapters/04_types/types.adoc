...
See <<__parameterized-types,Parameterized Types>> for details on that reference.
...
Function Type::
  Described in <<functions>>, <<_function_type>>.
Any Type::
  <<_any-type,Any Type>>.
...
Parameterized Type::
  Parameterized reference to defined generic class or interface; <<Parameterized-Types,Parameterized Types>>.
This Type::
...
constraint <<__use-site-structural-typing,Use-Site Structural Typing>>). Hence, the recommended
...
<<Function-Call-Constraints>> and <<Parameterized-Function-Call-Constraints>> do not apply.
...
_typeArgs[0]_ (cf. <<Array-Literal>>). Due to type erasure, the item type is not
