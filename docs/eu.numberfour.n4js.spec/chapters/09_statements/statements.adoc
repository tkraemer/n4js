...
126Dead Code[req:Dead_Code] For all statements in a function or field
accessor (getter/setter) body, the following constraints must hold:
...
//Requirement
//129For-In-Statement Constraints
[[req:For_In_Statement_Constraints]]
...
math:[\[\begin{aligned}
    ...
    \end{aligned}\]] Note that the expression may be evaluated to .
...
131With Statement[req:With Statement] With statements are not allowed in
N4JS or strict mode.
...
132Switch Constraints[req:Switch Constraints] For a given switch
statement math:[$s$], the following constraints must hold:
...
Cf. cite:[ECMA11a(13.2.1)], also http://www.2ality.com/2015/02/es6-scoping.html[Rauschmayer,
2ality: ``Variables and scoping in ECMAScript 6``]

=== Const

Cf. cite:[ECMA15a(13.2.1)], also http://www.2ality.com/2015/02/es6-scoping.html[Rauschmayer,
2ality: ``Variables and scoping in ECMAScript 6``]
...
//Requirement
//133Writability of const variables
[[req:Writability_of_const_variables]]
...
137Immutable Import[req:Immutable Import] Let math:[$i$] be a
binding to an imported element. It is an error if
