...
image::fig/diTerms.PNG[[fig:diTerms] key: pass the dependency instead of
letting the client create or find it]
...
image::fig/diagDICParentChild.png[title="Complex DIComponents Relations"]
...
140Bindings[req:Bindings] For a given binding
math:[$b=(key, target)$], the following constraints must
hold:
...
141Transitive Bindings[req:Transtive_Bindings] If an injector contains
two given bindings math:[$b_1=(key_1, target_1)$] and
...
//Requirement
//143Constructor Injection
[[req:Constructor_Injection]]
...
image::fig/injectionGraph_cycleField.png[image]
...
image::fig/injectionGraph_cycleCtorField.png[image]
...
image::fig/injectionGraph_cycleCtor.png[image]

 +

The injector needs to detect these cycles and resolve them.

144Resolution of Injection Cycles[req:Resolution_of_Injection_Cycles] A
cycle math:[$c \subset G$], with math:[$G$] being an injection
...
math:[$P$] annotated with `@Polyfill` or  `@StaticPolyfill`, that is
...
runtime-polyfill class math:[$P$] annotated with `@Polyfill`, that is
...
154Applying Polyfills[req:Applying_Polyfills]
task:IDE-1207[]
A polyfill is
...
polyfill class math:[$P$] annotated with `@StaticPolyfill`, that is
...
156Restrictions on static
polyfilling[req:Restrictions_on_static_polyfilling] For a static
polyfilling module math:[$M_P$] the following must hold:
