...
destructure them by using nested destructuring patterns. footnote:[Further reading on <<Acronyms,DI>> Basics: cite:[Fowler04b,Prasanna09a], Verification cite:[Zhu13a,Hudli13a], Frameworks cite:[Lesiecki08a,Betts13a,Knol13a,Dagger]]
...
between types (<<sec:N4JS_DI_Binders_and_Bindings>>).
...
For more information about bindings see <<_binders-and-bindings>>.
...
<<ex:complex-dicomponents-relations>> below.
...
[example]
--
<<ex:field-injection>> demonstrates simple field injection using default bindings. Note that all inherited fields
