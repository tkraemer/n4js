...
.  math:[$P$] must extend a class math:[$F$], math:[$F$]
is called the filled class:
+
$$P.super = F$$

. math:[$P$]’s name equals the name of the filled class and is
...
. math:[$P$] must not implement any interfaces:
$$P.implementedInterfaces=\emptyset$$
+
. math:[$P$] must have the same access modifier (access,
...
$$
+
.  If math:[$P$] declares a constructor, it must be override
compatible with the constructor of the filled class:
+
$$
\exists P.ownedCtor: P.ownedCtor <: F.ctor
$$
. math:[$P$] must define the same type variables as the filled
