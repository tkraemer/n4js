...
A complex Child-Parent relation between components is depicted in DIC Parent Child diagram and <<ex:complex-dicomponents-relations>> below.

[[fig-Complex DIComponents Relations]]
.Complex DIComponents Relations
image::{find}fig/diagDICParentChild.png[align=center]
...
methods (see <<_n4js-di-provides>>) which gives them a hook in instance creation process.
...
it into its own class footnote:[Cf. a blog post about micro types - http://www.markhneedham.com/blog/2009/03/10/oo-micro-types/, and tiny types - http://darrenhobbs.com/2007/04/11/tiny-types/ ].
...
[[fig-field-cycle]]
.Field Cycle
image::{find}fig/injectionGraph_cycleField.png[align=center]
...
[[fig-ctor-field-cycle]]
.Ctor Field Cycle
image::{find}fig/injectionGraph_cycleCtorField.png[align=center]
...
[[fig-ctor-cycle]]
.Ctor Cycle
image::{find}fig/injectionGraph_cycleCtor.png[align=center]
...
Since the second argument of the binding is declared as a singleton at the definition-site, this explicit binding implicitly ensures that the injector and all of its descendants preserve a singleton of the bound type `U`.
...
With this binding, each places where `J` is declared, type `U` is used at runtime. Furthermore, since `V` is declared as a singleton, both injector `F` and `G` are using a shared singleton instance of type `V`.
...
when configured with target <<_n4js-di-binder>>.
...
described in _injectable values_  <<_validation-of-callsites-targeting-n4injector-methods>>.
--
...
<<Req-IDE-72>>) and private members.
--
...
must hold in addition to <<Req-IDE-152>>:
...
related to runtime polyfilling described in <<_runtime-polyfill-definitions>> in a sense that both
...
hold in addition to <<Req-IDE-152>>:
