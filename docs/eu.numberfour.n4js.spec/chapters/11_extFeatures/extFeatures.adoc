...
1.  A subclass $S$ of $C$ is a <<Acronyms,DIC>> as well and it
...
creation and setting of values by an injector $I$ caused by
the creation of an root object $R$ the __injection phase__.
If an instance $C$ is newly created by the injector
$I$ (regardless of the injection point being used), the
injection is transitively applied on $C$. The following
...
exist. footnote:[Usually, only the itself is created like that, e.g., [language-n4js]`ar dic = N4Injector.of(DIC).create(DIC);}`]
...
to take place.
.  If $C.ctor$ is marked as injection point, all its
arguments are set by the injector. This is also true for an inherited
constructor marked as an injection point. See <<Req-IDE-143,requirement: Constructor Injection>> task:IDE-1264[]. For all arguments the injection phase constraints have to hold as well.
...
.  All fields of $C$, including _inherited_ once, marked as
...
$bind(key, target)$. It defines that for a dependency with a
given key which usually is the expected type at the injection point. An
instance of type $target$ is injected.
...
--
For a given binding $b=(key, target)$, the following constraints must
hold:
...
3.  If $b$ is implicit, then $key$ must be a
class. If $key$ references a type $T$, then
$target=T$ – even if $key$ is a use-site
structural type.
4.  $key$ and $target*$ can be nominal,
