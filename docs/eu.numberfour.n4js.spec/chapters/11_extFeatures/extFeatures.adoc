...
loops; called _destructuring binding_; see <<_var-statement,Var Statement>>).
...
what type should be injected into an _injection point_ (<<_n4js_di_injection_points,N4JS DI Injection Points>>) with an expected type.
...
parameters (with no further modifications): math:[\[\begin{aligned}
    ...
    \end{aligned}\]]
8.  All constraints related to member redefinition (cf. <<_redefinition_of_members,Redefinition of Members>>) have to hold.
