...
`acc` ::
Member access modifier as described in <<Accessibility-of-Members>>, it is the aggregated value of
...
method must not be overridden in subclasses; see <<Final-Methods>>.
...
<<Function-Type>> apply to methods as well. This section describes default values and
...
constraints for naming members of classifiers (cf. <<Member-Names>>)
and with the constraints detailed in the following sections on final
methods (<<Final-Methods>>), abstract methods (<<Abstract-Methods>>
...
implicit variable inside of their bodies (c.f. <<Arguments-Objects>>).
...
(cf. <<Abstract-Member>>). Private methods are implicitly declared
...
covered in <<Abstract-Member>> (see <<Abstract-Classes>>).
...
cf. <<Access-Super-Member-with-Super-Literal>>.
...
it may even be declared `@Final`, see <<Final-Methods>>.
...
constructor. Also see <<Assignment-Modifiers>>.
...
parameters through the implicit `arguments` variable inside of their bodies (c.f. <<Arguments-Objects>>).
...
Compare the following example to <<ex:Polymorphism_and_static_methods,Polymorphism and Static Methods>> for classes above:
