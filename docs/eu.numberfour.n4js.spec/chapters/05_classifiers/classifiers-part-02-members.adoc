...
--
For a constructor math:[$ctor$] of a class `C`, the following conditions
must hold:
...
1.  `C` has a direct super class math:[$C'$] and
...
..  math:[$I.constructor$] must be accessible from `C`,
...
constructor (owned or inherited, see <<covariant_constructor, definition: Covariant Constructor>>), we require the inherited constructor math:[$ctor$] of `C` within the context of
...
7.  The access modifier is compatible:
$$M.acc \geq S.acc$$
...
`S` and interfaces math:[$I_i$]) does not override or
...
--
Given a class
`C` and a superclass math:[$Sup$]. If for an instance
...
math:[$M, M'$] are an accessor pair for `S`:
...
`C`, and interface math:[$I_1,..., I_n$], implemented
by `C`, with
...
---
Given a classifier `C` footnote:[`C` could either be a class or an interface.], and interfaces math:[$I_1,..., I_n$]
implemented (or extended) by `C`, and sets math:[$M_C$]
and math:[$M_I$] as defined in . A non-static member
`M` defined in any interface math:[$I_i$] is merged
