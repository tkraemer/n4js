...
* in cases 1 and 4a an optional data field, getter, or setter of $R$ need not be present in $L$.
* in cases 2, 3, 4b, and 4c an optional getter of $R$ need not be present in $L$ and an optional field of $R$ requires only a setter in $L$.

If, according to these rules, a data field / getter / setter of $R$ need not be present in
$R$, but a member with the same name and access is actually present in $R$, that member in $R$
must be a data field / getter / setter of the same type / a subtype / a super type. In other
words, if a not actually required member is present in the subtype, ordinary rules for member
compatibility apply as if no optionality were involved (cf. general subtyping rules for
structural types).
