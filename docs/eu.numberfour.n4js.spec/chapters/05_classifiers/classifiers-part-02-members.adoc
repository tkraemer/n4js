...
$\hspace{2em}\exists f \in ctor.owner.ownedFields \to \tee s \subtype f $
...
* For every assign expression $assignExpr$ with $f.static \land assignExpr.left = T.f \to T=f.owner$.
* For every writing unary expression $u$ with $u.op \in \{++,--\}  \land   f.static \land  u.expression = T.f \to T=f.owner$.
...
+
[%hardbreaks]
$\mu(S)=\type{Method} \spc \to \mu(M) = \type{Method} $
$\mu(S)=\type{Field}  \spc \to \mu(M) \in \type{Field, Getter, Setter} $
$\mu(S)=\type{Getter} \spc \to \mu(M) \in \type{Field, Getter} $
$\mu(S)=\type{Setter} \spc \to \mu(M) \in \type{Field, Setter} $
...
[[def:Interface_and_Class_Member_Sets]]
...
--
Given a classifier `C` footnote:[`C` could either be a class or an interface.], and interfaces $I_1, \dots , I_n$ implemented (or extended) by `C`, and sets $M_C$ and $M_I$ as defined in <<def:Interface_and_Class_Member_Sets>>.
...
\\ TODO getter does no effect setter and vice versa
