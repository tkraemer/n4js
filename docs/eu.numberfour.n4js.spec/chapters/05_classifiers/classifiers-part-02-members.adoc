...
constraints for naming members of classifiers (cf. <<_member-names,Member Names>>)
...
implicit variable inside of their bodies (c.f. <<_arguments-objects,Arguments Objects>>).
...
In the code of <<_ex-covariant-constructors,Covariant Constructors example>>, we would get an error if we changed the type of parameter `p` in the constructor of `Cls2` to some other type that is not a super type of `B`,
