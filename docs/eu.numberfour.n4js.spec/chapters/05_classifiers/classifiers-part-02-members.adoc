...
the scope of application of this optionality is limited to a small number of specific use cases.
...
The detailed grammar is given in the sections for data fields and field accessors, respectively.
...
If a data field / getter / setter of a classifier C is optional, it need not be present in
a structural subtype of C, but if a member with the same name and access is present, it must
be a data field / getter / setter of the same type / a subtype / a super type (i.e. ordinary
type checks apply as if no optionality were involved).

However, this rule applies only to structural subtype checks in which the left-hand side of
the subtype relation is one of the following:
...
If none of the above cases 1 to 4 applies, an optional data field, getter, or setter is treated
as if it were not optional (i.e. the optionality is ignored).

In cases 1, and 4a the optionality applies to both getters and setters; in cases 2, 3, 4b, and 4c the optionality applies
only to getters, i.e. setters are treated as non-optional. In this respect, data fields are treated as getter/setter pairs.
...
h|                           5+^| &#916;                                | Case |
h|                              | `~` | `~~` | `\~w~` | `\~r~` | `\~i~` |      |
h|                           3+^| writable         2+^| not writable    |      |
h| `let x: &#916;C = {};`       |  ✓  |  ✓   |    ✓   |    ✓   |    ✓   |  1   | nothing required
h| `let x: &#916;C = new D0();` |     |      |        |    ✓   |    ✓   |  2   | setter required if writable
h| `let x: &#916;C = new DG();` |     |      |        |    ✓   |    ✓   |  2   | setter required if writable
h| `let x: &#916;C = new DS();` |  ✓  |  ✓   |    ✓   |    ✓   |    ✓   |  2   | setter required if writable
h| `let x: &#916;C = fooD0();`  |     |      |        |        |        | none | fails: D0 not final
h| `let x: &#916;C = fooSF0();` |     |      |        |        |        | none | fails: fooSF0() not nominal
h| `let x: &#916;C = fooF0();`  |     |      |        |    ✓   |    ✓   |  3   | ok, setter required if writable
|===

Here are the classes and functions used in the above table:
