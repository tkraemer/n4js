...
`acc` ::
Type access modifier as described in <<_accessibility-types-variables-functions>>, it is the aggregated value of the
...
footnote:[$accessorPair(m_1,m_2)$ is defined as follows: $(\mu(m_1)=\type{getter}\land \mu(m_2)=\type{setter}) \lor (\mu(m_1)=\type{setter}\land \mu(m_2)=\type{getter})$]
