...
runtime, see <<_runtime_definitions,Runtime Definitions>> for details.
...
external folders. This is shown in <<_externalClassImplementation_naming,Fig: External Class Implementation Naming>>.
...
runtime environment or runtime library (cf <<_runtime_environment_libraries,Runtime Environment Libraries>>).
