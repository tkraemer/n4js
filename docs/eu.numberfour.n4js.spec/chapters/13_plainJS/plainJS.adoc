...
below <<_implementation-of-external-declarations>>).
...
image::{find}fig/externalClassImplementation_naming.png[]
...
Since these cases are rather rare and in order to enable additional checks such as verification that a given runtime actually provides the elements, this kind of element can only be defined in components of type runtime environment or runtime library (cf <<_runtime-environment-and-runtime-libraries>>).
