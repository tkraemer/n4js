...
are allowed in a latexmath:[$n4jsd$] file.
...
//Requirement
//164External classes inheritance
...
//Requirement
//165Structurally typed interface implementation
...
//Requirement
//166External interface inheritance
...
//Requirement
//167External class/interface members
...
//Requirement
//168Other external declarations
...
//Requirement
[[req:169Implementation of External Declarations]]
//TODO : Requirements
task:IDE-1099[]
For a given external declaration latexmath:[$D$] but not for
...
provided in `Sources`, latexmath:[$n4jsd$] files in the folder tree
...
We introduce a new pseudo property latexmath:[$global$] on all declared
...
latexmath:[$global$] can be used without explicit import statement. It
is not possible to import these declarations.

//Requirement
//170Global Definitions
[[req:Global_Definitions]]

Global Definitions

For a declaration latexmath:[$D$] with latexmath:[$D.global=\TRUE$], not a polyfill
(latexmath:[$D.polyfill=\FALSE)$], the following constraints must hold:
...
We introduce a new pseudo property latexmath:[$providedByRuntime$]
accordingly:
...
//Requirement
//171Provided By Runtime
[[req:Provided_By_Runtime]]
Provided By Runtime

task:IDE-1036[]
For a declaration
latexmath:[$D$] with latexmath:[$D.providedByRuntime=\TRUE$], the
following constraints must hold:
...
type latexmath:[$\lenum{runtimeEnvironment}$] or
latexmath:[$\lenum{runtimeLibrary}$].
