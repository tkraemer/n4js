...
If, in addition to standard `source`, the `source-external` fragment is provided in `Sources`, $n4jsd$ files in the folder tree below source folders will be related to modules of the same name in the external folders. This is shown in <<fig-external-class-naming>>.
