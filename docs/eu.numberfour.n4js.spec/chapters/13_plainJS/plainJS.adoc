...
extension $n4jsd$ (so called N4JS definition files).
...
For a declaration $D$ with $D.global=\TRUE$, not a polyfill
($D.polyfill=\FALSE)$, the following constraints must hold:
...
We introduce a new pseudo property $providedByRuntime$
accordingly:
