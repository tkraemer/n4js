...
. Given a function or method _f_ with a declared return type _R_ that is declared `async`,
all return statements in _f_ must have an expression of type _R_ (not of type `Promise<R,?>`).
. `await` can be used in expressions directly enclosed in an async function,
and behaves like a unary operator with the same precedence as `yield` in ES6.
