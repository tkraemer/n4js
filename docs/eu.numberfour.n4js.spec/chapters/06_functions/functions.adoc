...
Default values of parameters are specified at a formal parameter of a function or method after the equal sign using an arbitrary initializer expression, such as `var = "s"`.
When a formal parameter has a declared type, the default value is specified at the end, such as: `var : string = "s"`.
The initializer expression is only evaluated in case no actual argument is given for the formal parameter.
However, the initializer expression is also evaluated when the actual argument value is `undefined`.

When a method is overwritten, its default parameters are not part of the overwriting method.
Consequently, default parameters in abstract methods are obsolete.
...
Instead, being optional is inferred from both default and variadic parameter modifiers.
There are two types of optional parameters, first: variadic parameters are always optional.
Second, default parameters can be optional parameters in case they are either specified at the end of the parameter list, or are followed by optional parameters only.
Default parameters followed by non-default, non-variadic parameters are never optional.
...
.  A function or method _f_ with a declared return type _R_ that is declared `*` has an actual return type of `Generator<TYield,TReturn,TNext>`.
...
. `yield* fg()` takes only generator functions or methods _fg_ with the actual return type `Generator<? extends TYield,? extends TReturn,? super TNext>`.
. The return type of the `yield*` expression is _TNext_.
--
