...
image::{find}fig/cdVarianceFunctionChart.png[]
...
** $F_{left} <: F_{right}$
(cf. <<function_type_conformance_non_parameterized>>)
* else if
...
and check the constraints from<<function_type_conformance_non_parameterized>> as if $F_{left}$ and
...
see <<Req-IDE-72>> (applies to method comnsumption and implementation accordingly,
see <<Req-IDE-73>> and <<Req-IDE-74>>).
...
This applies to all variable execution environments like field accessors (getter/setter, <<_field-accessors-getter-setter>>), methods (<<Methods>>) and constructors (<<_constructor-and-classifier-type>>), where `FormalParameter` type is used.
...
Constraints for function type are described in <<_function-type>>. Another consequence is that the inferred type of a function definition $fdecl$ is simply its function type $F$.
...
most details are explained in <<_ecmascript-5-function-definition>>.
...
function type as described in <<_function-type>>. Often, the signature of a function
...
[example]
--
In general, `{function():any}` is a subtype of `{function():void}` (cf. <<_function-type>>). When the return type of a function
