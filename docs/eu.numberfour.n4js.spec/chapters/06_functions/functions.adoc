...
//TODO check where this anchor goes
...
** $\exists \typeSubs: ( \typeEnvAdd \typeSubs ) \entails F_{left} <: F_{right}$ (cf. <<function_type_conformance_non_parameterized>>)
+
(i.e. there exists a substitution $\typeSubs$ of type variables in $F_{left}$ so that after substitution it becomes a subtype of $F_{right}$ as defined by <<function_type_conformance_non_parameterized>>)
...
and check the constraints from<<function_type_conformance_non_parameterized>> as if $F_{left}$ and $F_{right}$ were non-parameterized functions and, in
