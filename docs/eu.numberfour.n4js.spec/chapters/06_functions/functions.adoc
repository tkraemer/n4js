...
[[req:Upper_and_Lower_Bound_of_a_Function_Type]]
...
Annotation][ex:Function_Declaration_with_Type_Annotation]
...
The metatype of a function definition is function type (<<Function Type>>), as a function declaration is only a different syntax for creating a object.
Constraints for function type are described in <<Function Type>>. Another consequence is that the inferred type of a function definition math:[$fdecl$] is simply its function type
...
//Requirement
//83Function Declaration only on Top-Level
[[req:Function_Declaration_only_on_Top_Level]]
...
function type as described in <<Function Type>>. Often, the signature of a function
...
[[ex:Inference_Of_Function_Expression_s_Signature]]
...
//Requirement
//84no.this.in.top.level.arrow.function
// N4JS Mode
...
// N4JS Mode
//Requirements
//85no.arguments.in.top.level.arrow.function
...
[[ex:Async_as_keyword_and_identifier]]
...
//Requirement
//88@Promisify and await with promisifiable functions
[[req:Promisify_and_await__Promisifiable_function]]
