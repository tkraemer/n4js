...
//Requirement
//172@testee Semantics
[[req:_testee_Semantics]]

.  Tag can be only used on either
..  methods annotated with `@Test`
..  classes in test projects or folders, cf. <<jsdoc_tag__testeeFromType>>.
.  Tag requires a single argument, which is a fully qualified name to a
type, including the module specifier. Format is like this:
`pass:[moduleSpecifier '.' typeName ( ('.'|'#') memberName)?]`
.  Tag is _not_ repeatable, that is a single test method (or class) can
refer to only one testee. task:IDEBUG-635[]
