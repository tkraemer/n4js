package eu.numberfour.n4jsx.parser;
 ...
 */
// copied from eu.numberfour.n4js.parser.SemicolonInjectionHelper
public class SemicolonInjectionHelper {
	...
	public interface Callback {
		 ...
		 * eu.numberfour.n4js.tests.parser.ES_07_09_ASI_InfiniteLoopProblemTest.testInfiniteLoopBug()
		 */
	...
	public static void recover(IntStream inputStream, RecognitionException re, Callback callback) {
		...
		if (re instanceof MismatchedTokenException) {
			...
			if (expecting != InternalN4JSXParser.Semicolon) {
			...
			if (!isOffendingToken(la)) {
				...
				for (int ix = re.token.getTokenIndex() - 1; ix > 0; ix--) {
					...
					if (lt.getChannel() == Token.DEFAULT_CHANNEL) {
						...
						callback.recoverBase(inputStream, re);
						return;
					} else if (lt.getType() == InternalN4JSXParser.RULE_EOL) {
					...
					} else if (lt.getType() == InternalN4JSXParser.RULE_ML_COMMENT) {
	...
	public static boolean isOffendingToken(int la) {
		return la == InternalN4JSXParser.Semicolon
				|| la == InternalN4JSXParser.EOF
				|| la == InternalN4JSXParser.RightCurlyBracket
				|| la == InternalN4JSXParser.RULE_EOL
				|| la == InternalN4JSXParser.RULE_ML_COMMENT;
	}
	...
	private static boolean findCommaBeforeEOL(TokenStream casted, int startIndex) {
		for (int ix = startIndex - 1; ix > 0; ix--) {
			Token lt = casted.get(ix);
			if (lt.getType() == InternalN4JSXParser.Comma) {
	...
	static boolean isSemicolonEquivalent(Token lt) {
		if (lt.getType() == InternalN4JSXParser.RULE_EOL) {
			return true;
		}
		if (lt.getType() == InternalN4JSXParser.RULE_ML_COMMENT) {
			String tokenText = lt.getText();
