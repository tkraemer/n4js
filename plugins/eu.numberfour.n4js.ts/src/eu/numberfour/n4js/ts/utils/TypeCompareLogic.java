...
/* package */ class TypeCompareLogic {
	...
	/* package */ static int compare(IQualifiedNameProvider fqnProvider, TypeArgument arg1, TypeArgument arg2) {
		...
		if (ref1 instanceof ParameterizedTypeRef) {
			...
			c = compare(fqnProvider, cref1.getStaticTypeRef(), cref2.getStaticTypeRef());
			if (c != 0) {
				return c;
			}
		} else if (ref1 instanceof EnumTypeRef) {
			final EnumTypeRef eref1 = (EnumTypeRef) ref1;
			final EnumTypeRef eref2 = (EnumTypeRef) ref2;
			c = compare(fqnProvider, eref1.getEnumType(), eref2.getEnumType());
			if (c != 0) {
				return c;
			}
