...
// Rule TypeRef
ruleTypeRef :
	ruleTypeRefWithoutModifiers ruleUndefModifierToken? ruleNullModifierToken?
;
...
rulePrimitiveType :
	'primitive' ruleVoidOrBindingIdentifier (
		...
		'autoboxedType' ruleTypeReferenceName
	)? (
		'assignmnentCompatible' ruleTypeReferenceName
	)? '}'
;
...
ruleTObjectPrototype :
	...
	'@' RULE_IDENTIFIER
	) => ruleTAnnotation )* '{' ruleTMember* (
		ruleCallableCtor ruleTMember*
	)? '}'
;
...
ruleTClass :
	...
	ruleTypingStrategyDefSiteOperator? ruleBindingTypesIdentifier (
		...
		'implements' ruleParameterizedTypeRefNominal (
...
// Rule TFunction
ruleTFunction :
	ruleTypeAccessModifier 'providedByRuntime'? 'function' (
		'<' ruleTypeVariable (
			',' ruleTypeVariable
		)* '>'
	)? ruleBindingTypesIdentifier '(' (
