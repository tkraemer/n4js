 ...
 * generated by Xtext
 */
lexer grammar InternalRegularExpressionLexer;
...
RULE_CHARACTER_CLASS_ESCAPE : '\\' ('d'|'D'|'s'|'S'|'w'|'W');

RULE_CONTROL_ESCAPE : '\\' ('f'|'n'|'r'|'t'|'v');

RULE_CONTROL_LETTER_ESCAPE : '\\' 'c' ('a'..'z'|'A'..'Z')?;

RULE_HEX_ESCAPE : '\\' 'x' (RULE_HEX_DIGIT RULE_HEX_DIGIT?)?;

RULE_UNICODE_ESCAPE : '\\' ('u' (RULE_HEX_DIGIT (RULE_HEX_DIGIT (RULE_HEX_DIGIT RULE_HEX_DIGIT?)?)?|'{' RULE_HEX_DIGIT* '}'?)?)?;

RULE_DECIMAL_ESCAPE : '\\' RULE_DECIMAL_INTEGER_LITERAL_FRAGMENT;

RULE_IDENTITY_ESCAPE : '\\' .;

RULE_UNICODE_DIGIT : RULE_UNICODE_DIGIT_FRAGMENT;
