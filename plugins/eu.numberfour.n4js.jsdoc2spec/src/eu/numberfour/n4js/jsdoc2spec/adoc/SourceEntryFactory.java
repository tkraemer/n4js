...
import eu.numberfour.n4js.jsdoc2spec.RepoRelativePath;
import eu.numberfour.n4js.jsdoc2spec.SpecTestInfo;
...
public class SourceEntryFactory {
	...
	static final String EU_NUMBERFOUR = "eu.numberfour";
	...
	static private String getPath(RepoRelativePath rrp, String repository) {
		if (repository.equals(NO_REPO))
			return NO_PATH;
		int idxProject = rrp.pathInRepository.indexOf(EU_NUMBERFOUR);

		String tPath = rrp.pathInRepository.substring(0, idxProject);
		tPath = trim(tPath);
		return tPath;
	}
	...
	static private String getProject(RepoRelativePath rrp) {
		int idxProject = rrp.pathInRepository.indexOf(EU_NUMBERFOUR);
		int idxFolder = rrp.pathInRepository.indexOf("/", idxProject);

		String tProject = rrp.pathInRepository.substring(idxProject, idxFolder);
		tProject = trim(tProject);
		return tProject;
	}

	static private String getFolder(RepoRelativePath rrp, String module) {
		return getFolderInUriString(rrp.pathInRepository, module);
	}
	...
	static private String getFolderInUriString(String uriString, String module) {
		int idxProject = uriString.indexOf(EU_NUMBERFOUR);
	...
	static private String getExtension(RepoRelativePath rrp) {
		int extPointIdx = rrp.pathInRepository.lastIndexOf(".");
		String extension = rrp.pathInRepository.substring(extPointIdx);
		return extension;
	}
