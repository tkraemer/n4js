...
import eu.numberfour.n4js.jsdoc.dom.Text
import eu.numberfour.n4js.jsdoc.dom.VariableReference
import eu.numberfour.n4js.jsdoc.tags.DefaultLineTagDefinition
...
import static eu.numberfour.n4js.jsdoc.N4JSDocletParser.*
...
class ADocSerializer {
	...
	private def <T> StringBuilder appendApiConstraints(StringBuilder strb, Map<T, ? extends Collection<SpecTestInfo>> groupdTests,
		...
		for (group : groupdTests.entrySet.sortBy[keyToSortString.apply(it.key)]) {
			...
			strb.append("::");
			for (SpecTestInfo testSpec : group.value) {
				strb.append(" ");
