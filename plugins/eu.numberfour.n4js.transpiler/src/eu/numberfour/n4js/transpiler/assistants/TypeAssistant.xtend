 ...
 */
package eu.numberfour.n4js.transpiler.assistants
...
import eu.numberfour.n4js.transpiler.utils.ConcreteMembersOrderedForTranspiler
...
class TypeAssistant extends TransformationAssistant {
	...
	def public List<SymbolTableEntryOriginal> getSuperInterfacesSTEs(N4ClassifierDeclaration classifierDecl) {
		val superIfcRefs = switch(classifierDecl) {
		...
		.filter(SymbolTableEntryOriginal) // note: we also filer out null values here
		.toList;
	}
