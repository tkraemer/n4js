...
public class PatchedFollowElementComputer extends FollowElementComputer {
	...
	@Override
	public void computeFollowElements(Collection<FollowElement> followElements,
			final IFollowElementAcceptor followElementAcceptor) {
							...
							if (container instanceof Group) {
								...
								if (idx == group.getElements().size() - 1) {
									if (!currentState.contains(group) && GrammarUtil.isMultipleCardinality(group)) {
										...
										&& "?".equals(abstractElement.getCardinality())) { // loops are fine
			...
			if (newElement.getLookAhead() != element.getLookAhead()
				...
				if (newElement.getLookAhead() == element.getLookAhead()) {
					...
					if (newTrace.size() > originalTraceSize) {
						if (Collections.indexOfSubList(element.getLocalTrace(),
								newTrace.subList(originalTraceSize, newTrace.size())) != -1) {
							continue;
						}
					}
				}
