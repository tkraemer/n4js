 ...
 */
/* default */abstract class StringOperation {

	final static int[] EMPTY_REGIONS = new int[0];
	final static int MAX_OBVIOUS = 128;
	final static int[] OBVIOUS_IDENT_CHAR_NATURES = new int[MAX_OBVIOUS];

	final static int C_JLS_SPACE = 0x100;
	final static int C_SPECIAL = 0x80;
	final static int C_IDENT_START = 0x40;
	final static int C_UPPER_LETTER = 0x20;
	final static int C_LOWER_LETTER = 0x10;
	final static int C_IDENT_PART = 0x8;
	final static int C_DIGIT = 0x4;
	final static int C_SEPARATOR = 0x2;
	final static int C_SPACE = 0x1;
	...
	/**
	 * Answers all the regions in a given name matching a given <i>pattern</i> pattern (e.g. "H*M??"). </p>
	 * <p>
	 * Each of these regions is made of its starting index and its length in the given name. They are all concatenated
	 * in a single array of <code>int</code> which therefore always has an even length.
	 * </p>
	...
	private static char toLowerCase(char c) {
		if (c < MAX_OBVIOUS) {
			if ((OBVIOUS_IDENT_CHAR_NATURES[c] & C_LOWER_LETTER) != 0) {
				return c;
			} else if ((OBVIOUS_IDENT_CHAR_NATURES[c] & C_UPPER_LETTER) != 0) {
				return (char) (32 + c);
			}
		}
