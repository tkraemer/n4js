...
class N4JSExpressionValidator extends AbstractN4JSDeclarativeValidator {
	...
	def private void internalCheckTargetSubtypeOfDeclaredThisType(
	...
	private def void internalCheckAccessToStaticMemberOfInterface(
				...
				if (isStaticUtility && hasNameOfInterest) {
					...
					if (!tresult.failed) {
						...
						if (tr instanceof TypeTypeRef) {
							...
							val isReceiverPromise = if (str instanceof TypeRef) TypeUtils.isPromise(str,
									tscope) else false;
		...
		def checkNew(NewExpression newExpression) {
			...
			val isConcreteOrCovariant = !(typeArg instanceof Wildcard || typeArg instanceof ExistentialTypeRef ||
				typeArg instanceof ThisTypeRef) ||
				(staticType instanceof TClassifier &&
					N4JSLanguageUtils.hasCovariantConstructor(staticType as TClassifier));
			if (staticType === G.symbolObjectType) {
				...
				val message = IssueCodes.getMessageForEXP_NEW_WILDCARD_NO_COVARIANT_CTOR(typeArg.typeRefAsString,
					staticType.typeAsString);
				addIssue(message, newExpression, N4JSPackage.eINSTANCE.newExpression_Callee,
		...
		def internalCheckCtorVisibility(NewExpression expression, TypeTypeRef ref, Type staticType) {
			...
			if (ctorClassifier instanceof TClassifier) {
				...
				val vacs = new VisibilityAwareCtorScope(memberScope, memberVisibilityChecker, ref, staticType,
					expression);
			...
			def checkCastExpression(CastExpression castExpression) {
				...
				private def boolean internalCheckCastExpression(RuleEnvironment G, TypeRef S, TypeRef T,
					...
					if (T instanceof UnionTypeExpression) {
						...
						if (!internalCheckCastExpression(
							G,
							S,
							typeVariable.declaredUpperBound,
							castExpression,
							false,
				...
				private def boolean canCheck(RuleEnvironment G, TypeRef S, TypeRef T, boolean actualSourceTypeIsCPOE) {
					...
					def void checkIndexedAccessExpression(IndexedAccessExpression indexedAccess) {
						...
						val accessedStaticType = if (receiverTypeRef instanceof TypeTypeRef) tsh.getStaticType(G,
								receiverTypeRef);
...
receiverTypeRef.declaredType instanceof ContainerType<?> || receiverTypeRef instanceof ThisTypeRef)) {
							...
							if (isComputedName) {
								...
								if (!receiverTypeRef.isDynamic()) {
									addIssue(
										getMessageForEXP_INDEXED_ACCESS_N4CLASSIFIER(
											receiverTypeRef.declaredType.keyword), indexedAccess,
										EXP_INDEXED_ACCESS_N4CLASSIFIER);
									}
								}
								return
						 ...
						 */
						private def String getInvalidIndexType(RuleEnvironment G,
							IndexedAccessExpression indexedAccess) {
						...
						def boolean internalCheckIndexedAccessWithSymbol(RuleEnvironment G,
							IndexedAccessExpression indexedAccess, TypeRef receiverTypeRef,
							TField accessedBuiltInSymbol) {
								// check valid symbol (currently only 'iterator')
								if (accessedBuiltInSymbol !== G.symbolObjectType.ownedMembers.findFirst [
									name == "iterator"
								]) {
							...
							private def boolean holdsLefthandsideNotConst(IdentifierRef lhs) {
								...
								switch (id) {
									...
									TField case !id.writeable: {
										...
										addIssue(
											getMessageForVIS_WRONG_READ_WRITE_ACCESS("built-in constant", id.name,
												"read-only"), lhs, VIS_WRONG_READ_WRITE_ACCESS);
										return false;
									}
								}
								return true;
							...
							def checkPromisify(PromisifyExpression promiExpr) {
								if (!promisifyHelper.isPromisifiableExpression(promiExpr.expression)) {
									addIssue(getMessageForEXP_PROMISIFY_INVALID_USE, promiExpr,
										EXP_PROMISIFY_INVALID_USE);
								}
							}
