...
import eu.numberfour.n4js.validation.N4JSIssueSeverities
import eu.numberfour.n4js.validation.N4JSIssueSeveritiesProvider
import eu.numberfour.n4js.validation.helper.PolyFilledProvision
...
import static eu.numberfour.n4js.n4mf.N4mfPackage.Literals.*
		...
		references.filter(ProjectReference).forEach[
			...
			if (null !== id) {
				...
				if (null === project || null === project.projectType) {
					addNonExistingProjectIssue(it.eContainer, id, features.last, references.indexOf(it));
				} else {
