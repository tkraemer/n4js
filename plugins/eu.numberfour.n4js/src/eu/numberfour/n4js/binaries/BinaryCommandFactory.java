...
public class BinaryCommandFactory {
	...
	/** Instructs {@link ProcessExecutor} to execute external process without redirection of output to the caller */
	...
	public ProcessExecutionCommand createInstallPackageCommand(File installPath,
		...
		return new ProcessExecutionCommand() {
			...
			public ProcessResult execute() {
				...
				ProcessResult processResult = processExecutor.createAndExecute(processBuilder, COMMAND_NAME, SILENT);
		...
		return new ProcessExecutionCommand() {
			...
			public ProcessResult execute() {
				...
				ProcessResult processResult = processExecutor.createAndExecute(processBuilder, binary.getLabel(),
						/* used in internal checks, no need to pollute user console */ !SILENT);
				return processResult;
			}
		};
	}
