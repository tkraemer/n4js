...
class PolyProcessor_FunctionExpression extends AbstractPolyProcessor {
	...
	def package TypeRef processFunctionExpression(RuleEnvironment G, InferenceContext infCtx, FunctionExpression funExpr, TypeRef expectedTypeRef) {
		...
		if(!funExpr.isPoly) { // funExpr has declared types on all fpars and explicitly declared return type
			// can't use xsemantics here, because it would give us a ComputedTypeRef
			//return ts.type(G, funExpr).getValue();
			val result = TypeUtils.createFunctionTypeExpression(null, #[], fun.fpars,
				N4JSLanguageUtils.makePromiseIfAsync(funExpr, funExpr.returnTypeRef, G.predefinedTypes.builtInTypeScope)); // FIXME support for declared this type!!
		...
		for(var i=0;i<len;i++) {
			...
			if(fparAST.declaredTypeRef!==null) {
				...
				// undeclared type
				assertTrueIfRigid("type of formal parameter in TModule should be a DeferredTypeRef", fparT?.typeRef instanceof DeferredTypeRef);
		...
		if(funExpr.returnTypeRef!==null) {
			...
			assertTrueIfRigid("return type of TFunction in TModule should be a DeferredTypeRef", fun.returnTypeRef instanceof DeferredTypeRef);
			...
			returnTypeRef = N4JSLanguageUtils.makePromiseIfAsync(funExpr, returnTypeRef, G.predefinedTypes.builtInTypeScope);
		}
