...
/**
 * Scope implementation for intersection types. TODO: improve doc
 * <p>
 ...
 */
public class IntersectionMemberScope extends AbstractScope {

	private final IntersectionTypeExpression intersectionTypeExpression;
	...
	/**
	 * Creates intersection type scope, passed subScopes are expected to be fully configured (i.e., including required
	 * filters etc.)
	 */
	public IntersectionMemberScope(IntersectionTypeExpression intersectionTypeExpression, EObject context,
			List<IScope> subScopes,
			N4JSTypeSystem ts) {

		super(IScope.NULLSCOPE, false);

		this.intersectionTypeExpression = intersectionTypeExpression;
	...
	/**
	 * Returns all elements of intersection. No erroneous descriptions (instances of IEObjectDescriptionWithError) will
	 * be considered here, as we assume this method to be called from content assist and we do not want to show wrong
	...
	protected IEObjectDescription getSingleLocalElementByName(QualifiedName qualifiedName) {
		...
		TMember member = getOrCreateIntersectionMember(name);
	...
	private IEObjectDescription createUnionMemberDescriptionWithErrors(IEObjectDescription result) {
		return new UnionMemberDescriptionWithError(result, intersectionTypeExpression, subScopes, writeAccess);
	}

	/**
	 * For members of an intersection, pseudo instances need to be created (since the "intersection member" is either
	 * one of the members of the sub-elements or a new "merged" version). These pseudo instances are cached in the
	 * resource.
	 */
	private TMember getOrCreateIntersectionMember(String memberName) {
		// look up cache
		for (TMember currM : getCacheHolder(intersectionTypeExpression).getCachedComposedMembers()) {
			if (memberName.equals(currM.getName()) && hasCorrectAccess(currM, writeAccess)) {
				return currM;
			}
		}
		// not found, then create
		return createIntersectionMember(memberName);
	 ...
	 */
	private TMember createIntersectionMember(String memberName) {
		// check all subScopes for a member of the given name and
		// merge the properties of the existing members into 'composedMember'
		final IntersectionMemberDescriptor intersectionMemberDescr = new IntersectionMemberDescriptor(
				writeAccess, EcoreUtilN4.getResource(context, intersectionTypeExpression), ts);
		for (int idx = 0; idx < subScopes.length; idx++) {
			...
			final RuleEnvironment GwithSubstitutions = ts.createRuleEnvironmentForContext(
					intersectionTypeExpression.getTypeRefs().get(idx),
					EcoreUtilN4.getResource(context, intersectionTypeExpression));
			TMember member = findMemberInSubScope(subScope, memberName);
			intersectionMemberDescr.merge(GwithSubstitutions, member);
		}
		// produce result
		if (!intersectionMemberDescr.isEmpty()) {
			// at least one of the subScopes had an element of that name
			final TMember result;
			if (intersectionMemberDescr.isValid()) {
				// success case:
				// 1) A of the subScopes have an element for that name and
				// 2) they can be merged into a valid composed member
				result = intersectionMemberDescr.create(memberName);
			} else {
			...
			final ComposedTypeRef cacheHolder = getCacheHolder(intersectionTypeExpression);
